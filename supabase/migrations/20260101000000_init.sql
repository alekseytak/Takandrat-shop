-- Схема магазина ТАК И РАТ.
--
-- Откуда взялся файл: он лежал только в рабочей копии другой сессии
-- (.kilo/worktrees/, а этот каталог исключён из git) и был единственной
-- записью схемы. Схема перенесена в репозиторий, чтобы её можно было применить
-- к любому проекту, а не восстанавливать по памяти. Проект магазина
-- (xxkafurxhvcclwzabawm.supabase.co) при этом жив — проверено служебным ключом
-- 14 сентября; прежнее утверждение об удалении было ошибкой наблюдателя.
--
-- Политики RLS в этом файле слишком щедрые, их правит следующая миграция:
-- 20260914001000_rls_hardening.sql. Применять миграции по порядку.
