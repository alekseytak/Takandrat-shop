...
-- записью схемы. Проект Supabase, на который ссылается магазин
-- (xxkafurxhvcclwzabawm.supabase.co), больше не существует: имя хоста не
-- разрешается, тогда как сам supabase.com отвечает. Поэтому схема перенесена
-- в репозиторий — чтобы её можно было применить к новому проекту, а не
